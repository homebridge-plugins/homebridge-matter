...
export class OnOffLightAccessory extends BaseMatterAccessory {
  ...
  private async handleOnCommand(): Promise<void> {
    ...
    try {
      ...
      //   throw this.statusError('Timeout', 'Device did not respond - check network connection')
      // }
      // if (error.code === 'DEVICE_OFFLINE') {
      //   throw this.statusError('Failure', 'Device is offline or unreachable')
      // }
  ...
  private async handleOffCommand(): Promise<void> {
    ...
    try {
      ...
      //   throw this.statusError('Busy', 'Device is busy processing another command')
      // }
      // if (error.response?.status === 504) {
      //   throw this.statusError('Timeout', 'Request timed out after 30 seconds')
      // }
