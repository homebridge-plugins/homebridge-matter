...
export class RoboticVacuumAccessory extends BaseMatterAccessory {
  ...
  private async handleChangeCleanMode(request: MatterRequests.ChangeToMode): Promise<void> {
    ...
    //   throw this.statusError('InvalidAction', `Clean mode "${modeStr}" is not supported by this device`)
    // }
    ...
    //   throw this.statusError('InvalidInState', 'Cannot use mop modes - water tank not attached')
    // }
  ...
  private async handleSelectAreas(request: MatterRequests.SelectAreas): Promise<void> {
    ...
    //   throw this.statusError('NotFound', `Area ID(s) not found: ${invalidAreas.join(', ')}`)
    // }

    // Example: Check if vacuum supports area selection
    // if (!this.supportsAreaSelection) {
    //   throw this.statusError('InvalidAction', 'This vacuum does not support area selection')
    // }

    // Example: Prevent selecting areas while vacuum is running
    // if (this.operationalState === 1) { // 1 = Running
    //   throw this.statusError('InvalidInState', 'Cannot change area selection while vacuum is running')
    // }
